    ...
    override fun validateComplexBeforeModification(form: ContractForm, entity: CommissionEntity): Validation {
        ...
        if (record != null) {
            ...
            if (user.isAdmin()) {
                ...
                validateRequiredFieldWithValue(form.recruiter, "rekruter", user.id!!, "recruiter")
            }
            ...
            validation.addValidation(
                    validateRequiredDateNotBefore(form.startDate, "data rozpoczęcia pracy", record.startDate, "data umowy", "startDate")
            )
            validation.addValidation(
                    validateElectiveDateNotBefore(form.endDate, "data zakończenia pracy", record.startDate, "zamówiona data rozpoczęcia pracy", "endDate")
            )
            validation.addValidation(
                    validateElectiveDateNotAfter(form.endDate, "data zakończenia pracy", record.endDate, "zamówiona data zakończenia pracy", "endDate")
            )
            validation.addValidation(
                    validateRequiredNumberInRange(form.salary, "stawka", record.wageRateMin.toDouble(), record.wageRateMax.toDouble(), "commission")
            )
        }
...
private fun validateInitiallyCommon(form: ContractForm): Validation {
    ...
    validation.addValidation(
            validateRequiredDateNotAfter(form.contractDate, "data umowy", LocalDate.now(), "data dzisiejsza", "contractDate")
    )
    validation.addValidation(
            validateRequiredDateNotBefore(form.startDate, "data rozpoczęcia pracy", form.contractDate, "data umowy", "startDate")
    )
    validation.addValidation(
            validateElectiveDateNotBefore(form.endDate, "data zakończenia pracy", form.startDate, "data rozpoczęcia pracy", "startDate")
    )
    validation.addValidation(
