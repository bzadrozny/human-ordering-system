...
fun validateRequiredFieldWithValueWarning(field: Any?, name: String, expected: Any, fieldPath: String): ValidationDetails? {
    return if (field == null || field != expected) ValidationDetails(
...
fun validateRequiredNumberInRange(field: Number?, name: String, min: Double, max: Double, fieldPath: String): ValidationDetails? {
...
fun validateRequiredNumberBiggerThan(field: Number?, name: String, min: Double?, fieldPath: String, minName: String? = null): ValidationDetails? {
...
fun validateRequiredCollectionWithMinSize(field: Collection<Any>?, name: String, min: Int, fieldPath: String): ValidationDetails? {
...
fun validateElectiveCollectionWithMinSize(field: Collection<Any>?, name: String, min: Int, fieldPath: String): ValidationDetails? {
    return if (field != null && field.size < min) ValidationDetails(
            "Kolekcja '$name' jest wymagana i musi zawierać co najmniej $min elementów",
            fieldPath,
            ValidationStatus.BLOCKER
...
fun validateRequiredDateNotBefore(field: LocalDate?, name: String, minDate: LocalDate?, minDateName: String, fieldPath: String): ValidationDetails? {
...
fun validateElectiveDateNotBefore(field: LocalDate?, name: String, minDate: LocalDate?, minDateName: String, fieldPath: String): ValidationDetails? {
    return if (field != null && minDate?.let { field.isBefore(it) } == true) ValidationDetails(
            "Data '$name' nie może być wcześniejsza niż: '$minDateName'",
            fieldPath,
            ValidationStatus.BLOCKER
...
fun validateRequiredDateNotAfter(field: LocalDate?, name: String, maxDate: LocalDate?, maxDateName: String, fieldPath: String): ValidationDetails? {
    return if (field == null || maxDate?.let { field.isAfter(it) } == true) ValidationDetails(
            "Data jest obowiązkowa i '$name' nie może być późniejsza niż: '$maxDateName'",
            fieldPath,
            ValidationStatus.BLOCKER
    ) else null
}

fun validateElectiveDateNotAfter(field: LocalDate?, name: String, maxDate: LocalDate?, maxDateName: String, fieldPath: String): ValidationDetails? {
    return if (field != null && maxDate?.let { field.isAfter(it) } == true) ValidationDetails(
